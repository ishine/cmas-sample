...
#define KALDI_ERR std::cout
#define KALDI_ASSERT std::cout <<
...
SplitRadixComplexFft<Real>::SplitRadixComplexFft(int N)
{
	// if ((N & (N - 1)) != 0 || N <= 1)
...
template<typename Real>
SplitRadixComplexFft<Real>::SplitRadixComplexFft(const SplitRadixComplexFft<Real> &other)
	: N_(other.N_), logn_(other.logn_)
{
	...
	int lg2 = logn_ >> 1;
	if (logn_ & 1) lg2++;
	int brseed_size = 1 << lg2;
	brseed_ = new int[brseed_size];
	std::memcpy(brseed_, other.brseed_, sizeof(int) * brseed_size);

	if (logn_ < 4) {
		tab_ = NULL;
	} else {
		tab_ = new Real *[logn_ - 3];
		for (int i = logn_; i >= 4; i--) {
			int m = 1 << i, m2 = m / 2, m4 = m2 / 2;
			int this_array_size = 6 * (m4 - 2);
			tab_[i - 4] = new Real[this_array_size];
			std::memcpy(tab_[i - 4], other.tab_[i - 4], sizeof(Real) * this_array_size);
		}
	}
}
...
SplitRadixComplexFft<Real>::~SplitRadixComplexFft()
{
	delete[] brseed_;
	if (tab_ != NULL) {
		for (int i = 0; i < logn_ - 3; i++) delete[] tab_[i];
		delete[] tab_;
	}
}
...
template<typename Real>
void SplitRadixComplexFft<Real>::Compute(Real *x, bool forward, std::vector<Real> *temp_buffer) const
{
	...
	std::memcpy(static_cast<void *>(x + N_), static_cast<void *>(temp_ptr), sizeof(Real) * N_);

	Compute(x, x + N_, forward);
	// Now change the format back to interleaved.
	std::memcpy(static_cast<void *>(temp_ptr), static_cast<void *>(x + N_), sizeof(Real) * N_);
...
template<typename Real>
void SplitRadixComplexFft<Real>::BitReversePermute(Real *x, int logn) const
{
	int i, j, lg2, n;
	int off, fj, gno, *brp;
	Real tmp, *xp, *xq;
...
template<typename Real>
void SplitRadixComplexFft<Real>::ComputeRecursive(Real *xr, Real *xi, int logn) const
{
	int m, m2, m4, m8, nel, n;
